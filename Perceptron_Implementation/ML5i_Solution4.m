...
acc=perceptron1(fimage_train,flabel_train,fimage_test,flabel_test);
disp(acc);

   
function [accuracy] = perceptron1(img_train,lbl_train,img_test,lbl_test)
   w=zeros(1,784);
   b=0;
   a=0;   
   s=size(lbl_train);
   for i=1:size(lbl_train)
      accuracy(i)=0;
   end
     for j=1:size(img_train)
        a(j)=dot(img_train(j,:),w);
